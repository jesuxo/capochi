...
class SaprovController extends Controller
{
     ...
     */
    public function getComprasProveedor($codprov, Request $request, $limite = null)
    {
        $comercial = session('comercialid') ?: 1;

        $sucursales = Sasucursal::where('fk_comercial', $comercial)->pluck('id')->toArray();

        $query = Sacomp::with(['sucursal', 'items'])
            ->where('codprov', $codprov)
            ->whereIn('tipocom', ['H', 'I'])
            ->whereIn('fk_sucursal', $sucursales)
            ->orderBy('fechae', 'desc');
        ...
        foreach ($compras as $compra) {
            ...
            foreach ($compra->items as $item) {
                ...
                $totalUnidades += $item->cantidad * $compra->signo;
                $totalMonto += $item->preciod * $item->cantidad* $compra->signo;
            }
    ...
    public function debug($codprov, $codprod)
    {
        ...
        \Log::info('Seriales comprados a este proveedor para ' . $codprod, [
            'cantidad' => $serialesCompra->count(),
            'seriales' => $serialesCompra->pluck('nroserial')->toArray()
        ]);
        ...
        if (!empty($serialesList)) {
            ...
            \Log::info('Ventas de esos seriales para ' . $codprod, [
                'cantidad_ventas' => $ventas->count(),
                'ventas' => $ventas->toArray()
            ]);
        } else {
            \Log::info('No hay seriales comprados para ' . $codprod);
        }
        ...
        \Log::info('Ventas totales sin filtrar para ' . $codprod, [
            'unidades' => $ventasTotales
        ]);

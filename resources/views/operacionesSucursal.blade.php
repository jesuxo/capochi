...
@section('title')
  DETALLADO DE MERMA DE {{$sucursal}}
@endsection
@section('css')
...
@section('content')
    ...
    <div class="row">
        <form  method="post" name="form1" id="form1" action="/operaciones/detallado/sucursal">
            <div class="row">

                <div class="col-md-3  ">

                        <select class="form-select" data-choices onchange="window.document.getElementById('form1').submit()"
                                id="fk_sucursalaciones" name="fk_sucursal">
                            <option  {{(!$fk_sucursal)?'selected':''}} value="">TODAS LAS SUCURSALES</option>
                            @foreach($sucursales as $sucu)
                                <option  {{($sucu->id == $fk_sucursal)?'selected':''}} value="{{$sucu->id}}">{!!  $sucu->descrip  !!}</option>
                            @endforeach
                        </select>
        ...
        <div class="col-md-12 ">
            <div class="card-header mt-3 align-items-center justify-content-center text-center">
               DETALLADO DE MERMA DE {{isset($fk_sucursal)? $sucursal->descrip : ' TODAS LAS SUCURSALES '}}
                <br />
                ...
                <table  border="0" width="100%"  style="border-radius: 5px !important;" class="table table-borderless table-centered align-middle table-nowrap mb-0 mt-3" >
                    ...
                    @foreach($itemopei as $index => $item)
                        ...
                        <tr bgcolor="#f5f5f5">
                            ...
                            @foreach($operaciones as $indexoper  => $oper)
                        ...
                        @foreach($item as $index2 => $productos)
                            ...
                            <tr @php if(($n%2)!=0){  echo 'bgcolor="#eeeeee"';} @endphp>
                                ...
                                @foreach($operaciones as $indexs => $vals)
                                    @php
                                        $key = $index2.$indexs;
...
@section('scripts')
    ...
    <script>
        var productCategoryInput = new Choices('#fk_sucursalaciones', {
            searchEnabled: false,
            shouldSort: false,
        });
    </script>

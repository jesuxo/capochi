...
@section('css')
    <style>
        ...
        .sucursal-link:hover {
            background-color: rgba(249, 90, 2, 0.1);
            color: #f95a02  !important;
                                ...
                                @php
                                    $sql = "
                                        SELECT
                                            ...
                                            sum(IFNULL(
                                                (select ((contado+credito)/tasa_dolar)  as totalmontodivisa
                                                 from safact g
                                    ...
                                    <td>
                                        <a href="{{route('saacxc',['id'=>$sucu->id, 'fechasreport'=>$fechasreport])}}"
                                           class="sucursal-link text-dark" style="font-size: 12px">
                                            <i class="bi bi-shop me-1"></i>{{  $sucu->descrip  }}
                                            <i class="bi bi-box-arrow-up-right ms-1" style="font-size: 10px; margin-left: 12px !important;  color: #f95a02;"></i>
                                        </a>
                                    </td>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body p-0">
                        ...
                        <div class="px-3 pt-2 pb-2 d-flex justify-content-between align-items-center border-bottom">
                            <div>
                                <span class="badge bg-primary" id="totalClientes">{{ count($saldocxc) }}</span>
                            ...
                            <table class="table table-sm table-hover table-cxc mb-0">
                                ...
                                @foreach($saldocxc as $index => $cxc)
                                    ...
                                    <tr class="cliente-row" data-cliente="{{ strtolower($cxc->cliente) }}" data-codclie="{{ $cxc->codclie }}">
                                        ...
                                        <td class="text-center">
                                            <span class="badge bg-primary">{{ number_format($cxc->deudas, 0, ',', '.') }}</span>
                                        </td>
                                        ...
                                        <td class="text-end text-danger fw-bold">{{ number_format($cxc->saldo, 2, ',', '.') }}</td>
                                         <td class="text-center">
                                            <button type="button"
                                                    class="btn btn-sm btn-outline-primary cxcmodal"
                                ...
                                @if(count($saldocxc) > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end text-primary fw-bold"></td>
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content" id="contentdescuento">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-warning" id="btnAplicarDescuento" onclick="$(this).hidePopover()">
...
// Calcular preview del descuento
            ...
            $(document).on('keyup change', '#montoDescuento, #porcentajeDescuento', function() {
                calcularPreview();
            });
...
// Aplicar descuento
    ...
    `;
        ...
        function calcularTotalUSD() {
            ...
            if(tasaBs > 0){
                ...
                const saldo = {{ $tsaldo ?? 0 }};
                $('#totalUSD').text('$' + total.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ','));

                if (total > saldo) {
                    $('#totalUSD').addClass('text-danger');
                    $('#alertMonto').text('El monto total excede el saldo de la factura');
                } else if (total <= 0) {
    ...
    `);
        ...
        $(document).on('click', '.openDocumentoEnLinea', function(e) {
            e.preventDefault();
            e.stopPropagation();
            verDocumento(
                $(this).data('fksucu'),
